...
def presAbs(calls, listgenomesRemove, outputfile, cgPercent):
    ...
    def binarize(column):
        ...
        fix_inf = column.replace(to_replace='\*', value='', regex=True)
...
def clean(inputfile, outputfile, toremovegenes, toremovegenomes, cgPercent):
    ...
    #need to replace the * alleles for a high number, simple remove the * will make alleles to be repeated while not being the same
    #iterate through columns and get uniques

    for column in cleaned:
        maxAllele=999999
        list_uniques=cleaned[column].unique()
        for elem in list_uniques:
            if "*"in str(elem):
                cleaned[column] = cleaned[column].replace(elem,str(maxAllele))
                maxAllele -= 1


    #cleaned = cleaned.replace(to_replace='\*', value='', regex=True)
