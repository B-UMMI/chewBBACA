...
def get_Short (gene,auxBar,maxBsrShort):
	blastPath='blastp'
	genesList=[str(gene)]
	maxBsrShort=float(maxBsrShort)
	...
	for gene in genesList:
		...
		for allele in SeqIO.parse(gene, "fasta", generic_dna):
			try:
				...
				alleleI=str(((allele.name).split("_"))[-1])
				if counter<1:
					...
					Gene_Blast_DB_name2 = CommonFastaFunctions.Create_Blastdb_no_fasta(tempgeneProt2, 1, True,tempgeneProt2Fasta)


					# --- get BLAST score ratio --- #
					cline = NcbiblastpCommandline(cmd=blastPath, db=Gene_Blast_DB_name2, evalue=0.001, outfmt=5,num_threads=1)
					...
					if bestbsr<maxBsrShort:
						...
						Gene_Blast_DB_name = CommonFastaFunctions.Create_Blastdb_no_fasta(tempgeneProt, 1, True,
																						  tempgeneProtFasta)
...
def main(geneFiles,cpu2use,maxBsrShort):
	...
	for gene in listGenes:
		...
		pool.apply_async(get_Short,args=[str(gene),auxBar,maxBsrShort])
