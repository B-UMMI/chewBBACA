...
def down_prof():
    ...
    parser.add_argument('--sc', nargs='?', type=str, help='tsv with profile', required=True)
    parser.add_argument('--cpu', nargs='?', type=int, help='number of cpu', required=False, default=1)
    ...
    species = args.sp
    schema = args.sc
    cpu2use = str(args.cpu)

    

    ScriptPath = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'utils/down_profiles.py')
    args = [ScriptPath, '--sp', species, '--sc', schema, '--cpu', cpu2use  ]
